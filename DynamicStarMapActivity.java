...
public class DynamicStarMapActivity extends InjectableActivity
  ...
  private void initializeModelViewController() {
    ...
    // ✅ Access the SkyGradientLayer (if needed later)
    SkyGradientLayer gradientLayer = layerManager.getSkyGradientLayer();
    if (gradientLayer != null) {
      gradientLayer.setVisible(false); // Hide it when camera turns ON
    }

    // Attach the renderer controller
    rendererController = new RendererController(renderer, skyView);
    rendererController.addUpdateClosure(new RendererModelUpdateClosure(model, rendererController, sharedPreferences));
  ...
  private void startCamera() {
    ...
    try {
      if (ActivityCompat.checkSelfPermission(this, Manifest.permission.CAMERA)
        ...
        Log.w("CameraDebug", "Camera permission not granted");
        return;
      }
      ...
      if (cameraDevice != null) {
        Log.d("CameraDebug", "Camera already open");
        return;
      }
      ...
      manager.openCamera(cameraId, new CameraDevice.StateCallback() {
        ...
        public void onOpened(@NonNull CameraDevice camera) {
          Log.d("CameraDebug", "Camera opened");
          cameraDevice = camera;
        ...
        @Override public void onDisconnected(@NonNull CameraDevice camera) {
          Log.w("CameraDebug", "Camera disconnected");
          camera.close();
          cameraDevice = null;
        }
      ...
      Log.e("CameraDebug", "startCamera failed", e);
    }
  }
  ...
  private void createCameraPreviewSession() {
    try {
      ...
      if (texture == null) {
        Log.w("CameraDebug", "SurfaceTexture is null!");
        return;
      }
      ...
      cameraDevice.createCaptureSession(Arrays.asList(surface), new CameraCaptureSession.StateCallback() {
        ...
        public void onConfigured(@NonNull CameraCaptureSession session) {
          ...
          try {
            ...
            Log.e("CameraDebug", "setRepeatingRequest failed", e);
          }
        }

        @Override
        public void onConfigureFailed(@NonNull CameraCaptureSession session) {
          Log.e("CameraDebug", "Camera preview session failed");
        }
      }, null);

    } catch (CameraAccessException e) {
      Log.e("CameraDebug", "createCameraPreviewSession failed", e);
    }
  }

  private void stopCamera() {
    Log.d("CameraDebug", "stopCamera() called");
